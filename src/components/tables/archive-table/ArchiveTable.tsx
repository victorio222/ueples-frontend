...
// import { Document } from "../../../types/models"; // Ensure this matches your model

// interface StudentArchiveTableProps {
//   selectedYear?: string; // The label (e.g., "2025-2026")
//   yearId?: number;       // The numeric ID from the database
// }

// export default function StudentArchiveTable({ selectedYear, yearId }: StudentArchiveTableProps) {
//   const [searchTerm, setSearchTerm] = useState("");
//   const navigate = useNavigate();
...
//       try {
//         setLoading(true);
//         const response = await API.docs.getByYear(yearId);
//         // Note: Check if your backend uses .data.data or just .data
...
//     return studentName.includes(searchTerm.toLowerCase()) || lrn.includes(searchTerm);
//   });
...
//             <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
//               <TableRow>
//                 <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs">Student Name</TableCell>
//                 <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs">Document Type</TableCell>
//                 <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs">Status</TableCell>
//                 <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs">Date Uploaded</TableCell>
//                 <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-end text-theme-xs">Actions</TableCell>
//               </TableRow>
//             </TableHeader>

//             <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
//               {loading ? (
//                 <TableRow><td colSpan={5} className="py-10 text-center text-gray-500">Loading documents...</td></TableRow>
//               ) : filteredData.length > 0 ? (
//                 filteredData.map((doc) => (
//                   <TableRow key={doc.document_id} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
//                     <TableCell className="px-5 py-4 text-start">
//                       <div className="flex items-center gap-3">
...
//                            <span className="text-xs font-bold text-gray-400">{doc.Student?.last_name[0]}</span>
//                         </div>
//                         <div>
//                           <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
//                             {doc.Student?.first_name} {doc.Student?.last_name}
//                           </span>
//                           <span className="block text-gray-500 text-theme-xs">LRN: {doc.Student?.lrn}</span>
//                         </div>
//                       </div>
//                     </TableCell>
//                     <TableCell className="px-4 py-3 text-gray-500 text-theme-sm">{doc.type}</TableCell>
//                     <TableCell className="px-4 py-3 text-start">
...
//                        <div className="flex items-center justify-end gap-2">
//                         <a 
//                           href={`http://localhost:5000/${doc.attachment}`} 
//                           target="_blank" 
//                           rel="noreferrer"
...
//                     {error || `No student records found for A.Y. ${selectedYear}`}
//                   </td>
//                 </TableRow>
//               )}
//             </TableBody>
...
interface StudentArchiveTableProps {
  selectedYear: string; // The label from URL params (e.g., "2025-2026")
}
...
export default function StudentArchiveTable({ selectedYear }: StudentArchiveTableProps) {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      ...
      try {
        setLoading(true);
        // Using the acad_year string as expected by your fetchByAcademicYear controller
              ...
              {loading ? (
                ...
                filteredData.map((doc) => (
                  <TableRow key={doc.document_id} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.01]">
                    <TableCell className="px-5 py-4 text-start">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 overflow-hidden rounded-full bg-gray-100 flex items-center justify-center font-bold text-gray-400">
