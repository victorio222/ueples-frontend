...
//   const [isProcessing, setIsProcessing] = useState(false); // For PDF generation feedback
...
//       let response;
//       if (lrnFilter) {
//         // If LRN is selected, call the filtered endpoint
//         // Adjust endpoint based on your API (e.g., DocumentService.getByStudent)
//         response = await DocumentService.getAll(page, pageSize, lrnFilter); 
//       } else {
//         // Otherwise, fetch recent uploads as usual
//         response = await DocumentService.getAll(page, pageSize);
//       }
...
//     // Listen for filter changes from the Input component
...
//   // const fetchRecentFiles = async (page: number) => {
//   //   try {
//   //     setLoading(true);
//   //     const response = await DocumentService.getAll(page, pageSize);
//   //     const { documents, totalPages, totalItems } = response.data;
...
//     img.onload = () => {
//       // Form 137 is often Legal size (8.5 x 13 in the Philippines)
...
//       // Calculate width with 0.5 inch margins
//       const pdfWidth = pageWidth - 1; 
//       const pdfHeight = pdfWidth / ratio;

//       pdf.addImage(img, "JPEG", 0.5, 0.5, pdfWidth, pdfHeight);
//       pdf.save(`Form137_${upload.student.last_name}.pdf`);
...
//         <html>
//           <head><title>Print Form 137</title></head>
...
//                 Uploaded By
//               </TableCell>
//               <TableCell isHeader className="text-center px-5 py-3 font-medium text-gray-500 text-theme-xs dark:text-gray-400">
//                 Actions
//               </TableCell>
//             </TableRow>
//           </TableHeader>
...
//                       {upload.type}
//                     </span>
//                     <span className="block text-gray-400 text-theme-xs truncate max-w-[150px]">
...
//                       {/* VIEW ORIGINAL */}
//                       <a
//                         href={`${API_BASE_URL}/${upload.attachment.replace(/\\/g, "/")}`}
//                         target="_blank"
//                         rel="noreferrer"
//                         className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-blue-600 dark:hover:bg-white/5"
//                         title="View Original"
//                       >
...
//                         className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-green-600 dark:hover:bg-white/5"
...
//                       {/* PRINT */}
//                       <button
//                         onClick={() => handlePrint(upload.attachment)}
//                         className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-purple-600 dark:hover:bg-white/5"
...
//           Page <span className="font-medium text-gray-700 dark:text-white">{currentPage}</span> of {totalPages}
//         </p>

//         <div className="flex items-center gap-1">
//           {currentPage > 3 && (
//             <button onClick={() => setCurrentPage(1)} className="px-2 py-1 text-xs text-gray-400 hover:text-blue-600">First</button>
//           )}
...
//             <button onClick={() => setCurrentPage(totalPages)} className="px-2 py-1 text-xs text-gray-400 hover:text-blue-600">Last</button>
//           )}
//         </div>
//       </div>
//     </div>
...
export default function RecentUploadsTable() {
  ...
  const fetchRecentFiles = useCallback(async (page: number, lrnFilter: string | null) => {
    try {
      setLoading(true);
      // Using the updated service that accepts 3 arguments
  ...
  return (
    <div className="space-y-4">
      {/* Title indicating filter status */}
