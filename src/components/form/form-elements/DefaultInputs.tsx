...
import api from "../../../utils/axiousInstance";
import ReactSelect from "react-select";
import { showAlert } from "../../../utils/toaster"; // Import the helper
...
export default function DefaultInputs() {
  ...
  useEffect(() => {
    const fetchFormData = async () => {
      try {
        ...
        showAlert.error("Initialization Error", "Failed to load student or year options.");
      }
    };
    fetchFormData();
  }, []);
  ...
  const selectStyles = {
    input: (base: any) => ({ ...base, "gridArea": "1/1/2/3" }),
    control: (base: any) => ({ ...base, boxShadow: 'none' }),
    indicatorSeparator: () => ({ display: 'none' }),
  };
  ...
  const handleSubmit = async () => {
    if (!lrn || !yearId || !file) {
      showAlert.error("Missing Info", "Please fill in all required fields and upload a file.");
      return;
    }
    ...
    try {
      setLoading(true);
      // 1. Show Loading UI
      showAlert.loading("Uploading and archiving document...");

      await DocumentService.upload(formData);

      // 2. Success Alert
      await showAlert.success("Success!", "Student record has been archived successfully.");

      window.dispatchEvent(new Event("documentUploaded"));
      setFile(null); 
    } catch (error: any) {
      // 3. Error Alert
      const errorMessage = error.response?.data?.message || "Something went wrong during upload.";
          ...
          {file && (
            <div className="mt-4 p-3 border border-dashed border-blue-500 rounded-lg bg-blue-500/5 flex items-center justify-between">
               <div className="flex items-center gap-3">
                ...
                <p className="text-sm dark:text-white/90 font-medium">{file.name}</p>
               </div>
               <button 
